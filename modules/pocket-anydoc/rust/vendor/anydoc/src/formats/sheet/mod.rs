...
const MAX_MOBILE_SPREADSHEET_CELLS: usize = 250_000;
const MAX_MOBILE_MERGED_REGIONS: usize = 4_096;
...
pub fn parse(bytes: &[u8]) -> Result<Document, ConvertError> {
    ...
    for name in &sheet_names {
        ...
        let regions = merged
            ...
            .map(Vec::as_slice)
            .unwrap_or_default();
        if regions.len() > MAX_MOBILE_MERGED_REGIONS {
            return Err(spreadsheet_cell_limit(
                regions.len(),
                MAX_MOBILE_MERGED_REGIONS,
            ));
        }
        let mut covered_budget = 0usize;
        for d in regions {
            crate::runtime::charge(1)?;
            ...
            let region_cells = (r1 - r0).saturating_mul(c1 - c0);
            covered_budget = covered_budget.saturating_add(region_cells);
            if covered_budget > MAX_MOBILE_SPREADSHEET_CELLS {
                return Err(spreadsheet_cell_limit(
                    covered_budget,
                    MAX_MOBILE_SPREADSHEET_CELLS,
                ));
            }
        ...
        for (r, row) in range.rows().enumerate() {
            crate::runtime::charge(row.len().max(1))?;
            builder.next_row();
...
fn merged_regions<RS: std::io::Read + std::io::Seek>(
    ...
    for name in sheet_names {
        ...
        match regions {
            ...
            Ok(_) => {}
            Err(e) => {
                if let Some(limit) = map_mobile_limit(&e) {
                    return Err(limit);
                }
                log::warn!("skipping unreadable merged-region list for {name:?}: {e}");
            }
        }
    }
    Ok(out)
...
fn map_open_error(e: calamine::Error) -> ConvertError {
    ...
    } else if let Some(limit) = map_mobile_limit(&text) {
        limit
    } else {
...
fn map_mobile_limit(text: &str) -> Option<ConvertError> {
    text.contains(calamine::MOBILE_CELL_LIMIT_MESSAGE).then(|| {
        spreadsheet_cell_limit(
            MAX_MOBILE_SPREADSHEET_CELLS + 1,
            MAX_MOBILE_SPREADSHEET_CELLS,
        )
    })
}

fn spreadsheet_cell_limit(requested: usize, max: usize) -> ConvertError {
    ConvertError::ResourceLimit {
        limit: "max_spreadsheet_cells",
        detail: format!("worksheet requested {requested} cells or regions; max {max}"),
    }
}

fn format_data(data: &Data) -> Result<String, ConvertError> {
    ...
    if value == calamine::UNSUPPORTED_FORMAT_SENTINEL {
        Err(ConvertError::ResourceLimit {
            limit: "unsupported_spreadsheet_format",
            detail: "a used numeric cell has an unsupported display format".into(),
        })
    } else {
        Ok(value)
    }
...
mod tests {
    ...
    fn string_cells_are_not_trimmed() {
        assert_eq!(
            format_data(&Data::String("  padded  ".into())).unwrap(),
            "  padded  "
        );
    }
