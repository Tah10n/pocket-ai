...
use crate::{
    ...
    HeaderRow, Metadata, Range, Reader, ReaderRef, Sheet, SheetType, SheetVisible, Table,
    MOBILE_CELL_LIMIT_MESSAGE, MOBILE_MAX_MERGED_REGIONS, MOBILE_MAX_WORKSHEET_CELLS,
};
...
impl std::fmt::Display for XlsxError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            XlsxError::CellLimitExceeded { requested, max } => write!(
                f,
...
fn ensure_mobile_vector_len(len: usize) -> Result<(), XlsxError> {
    if len > MOBILE_MAX_WORKSHEET_CELLS {
        Err(XlsxError::CellLimitExceeded {
            requested: len,
...
fn ensure_mobile_sparse_range<T: crate::CellType>(cells: &[Cell<T>]) -> Result<(), XlsxError> {
    mobile_sparse_range_limit(cells).map_err(|requested| XlsxError::CellLimitExceeded {
        requested,
        max: MOBILE_MAX_WORKSHEET_CELLS,
    })
}
        ...
        match xml.read_event_into(&mut buffer) {
            Ok(Event::Start(event)) if event.local_name().as_ref() == b"mergeCell" => {
                if let Some(val) = event.raw_attr(b"ref")? {
                    if merge_cells.len() >= MOBILE_MAX_MERGED_REGIONS {
                        return Err(XlsxError::CellLimitExceeded {
                            requested: merge_cells.len().saturating_add(1),
                            max: MOBILE_MAX_MERGED_REGIONS,
                        });
                    }
                    let dimensions = get_dimension(val)?;
