...
| `stop`, `n_probs` | New request settings. Stops are copied and combined with template/safety stops without trimming significant whitespace. Probabilities are opt-in. | No reload. At most 32 bounded user strings; `n_probs` defaults to `0`, capped at 10. Final diagnostic retention is capped at 64 tokens and 256 characters per token string. No growing probability arrays are copied into each streamed chat update. | Engine whitespace, immutable-input and probability-bound tests; real probability output still requires native evidence. |
| `ignore_eos`, `logit_bias` | Typed settings recognize these fields, but the engine refuses `ignore_eos: true` and nonempty bias lists before native. | **Blocked by a pinned native defect**, not a supported toggle. The app accepts only numeric token-ID/numeric-bias pairs for representation; string token names and boolean bias entries are not added by casts. | Installed `JSIParams.cpp` exactly matched the tagged source during review; see the defect below. Dangerous behavior was not executed. |
...
### Pinned unsafe sampler paths

In [the exact rc.3 JSI parameter reader](https://github.com/mybigday/llama.rn/blob/v0.13.0-rc.3/cpp/jsi/JSIParams.cpp#L628),
`sparams.logit_bias.clear()` is followed by indexing that cleared container using
an EOS/token ID. The member is a `std::vector<llama_logit_bias>` in the selected
native declarations. Both `ignore_eos` and nonempty `logit_bias` enter that unsafe
path. The app blocks them without upgrading the runtime or changing native code.
The source comments also advertise string tokens/boolean biases that do not match
the public TypeScript number-array contract and the bridge's numeric token read.
Neither comments nor a type cast are compatibility evidence.

### Acceptance status

The new paths have automated JavaScript/type tests, including deferred native-call
mocks. These establish mapping and ownership behavior, not native model support.
The compatible fixture candidates and provenance are recorded in
[LoRA fixture research](validation/llama-rn-stage3/lora-fixture.md). That research
does not prove application or output effect. At this inventory update the new
binary's ordinary/stop, JSON object/schema, GBNF, custom-template/prefill,
probabilities, real LoRA effect, A+LoRA → auxiliary B → A, and adapter-deletion
scenarios remain **not_run** until an acceptance record supplies the APK/source,
model/adapter identities and result. Prior-stage receipts are historical evidence,
not a substitute for repeating them on the new binary. iOS/GPU/NPU and unavailable
MTP fixtures are independent checks, never inferred from an Android CPU result.
