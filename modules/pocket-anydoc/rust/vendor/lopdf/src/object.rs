...
/// Pocket AI's per-stream mobile decompression ceiling. This is enforced in
/// lopdf itself so object streams are bounded during `Document::load`, before
/// a higher-level caller can inspect the document.
pub const MAX_DECOMPRESSED_STREAM_BYTES: usize = 2 * 1024 * 1024;
...
impl Stream {
    ...
    pub fn decompressed_content(&self) -> Result<Vec<u8>> {
        if self.content.len() > MAX_DECOMPRESSED_STREAM_BYTES {
            return Err(Error::ResourceLimit("max_pdf_stream_bytes"));
        }
    ...
    fn decompress_lzw_loop(
        ...
        impl std::io::Write for LimitedWriter {
            fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
                if self.bytes.len().saturating_add(buf.len()) > MAX_DECOMPRESSED_STREAM_BYTES {
                    self.limit_hit = true;
                    return Err(std::io::Error::other("PDF stream output limit"));
                }
        ...
        let mut output = LimitedWriter {
            bytes: Vec::with_capacity(input.len().min(MAX_DECOMPRESSED_STREAM_BYTES)),
            limit_hit: false,
        };
    ...
    fn decompress_zlib(input: &[u8], params: Option<&Dictionary>) -> Result<Vec<u8>> {
        ...
        fn read_bounded(reader: &mut impl Read) -> Result<Vec<u8>> {
            let mut output = Vec::with_capacity(MAX_DECOMPRESSED_STREAM_BYTES.min(64 * 1024));
            ...
            loop {
                ...
                if output.len().saturating_add(read) > MAX_DECOMPRESSED_STREAM_BYTES {
                    return Err(Error::ResourceLimit("max_pdf_stream_bytes"));
                }
        ...
        if !input.is_empty() {
            ...
            match read_bounded(&mut decoder) {
                Ok(decoded) => output = decoded,
                Err(Error::ResourceLimit(limit)) => return Err(Error::ResourceLimit(limit)),
                Err(err) => {
                warn!("{err}");
                ...
                if input.len() > 2 {
                    ...
                    match read_bounded(&mut raw_decoder) {
                        Ok(decoded) => output = decoded,
                        Err(Error::ResourceLimit(limit)) => {
                            return Err(Error::ResourceLimit(limit));
                        }
                        Err(raw_err) => {
    ...
    fn decode_ascii85(input: &[u8]) -> Result<Vec<u8>> {
        ...
        for &ch in input_no_eod {
            if ch == b'z' {
                ...
                if output.len().saturating_add(4) > MAX_DECOMPRESSED_STREAM_BYTES {
                    return Err(Error::ResourceLimit("max_pdf_stream_bytes"));
                }
        ...
        if count > 0 {
            ...
            if output.len().saturating_add(count - 1) > MAX_DECOMPRESSED_STREAM_BYTES {
                return Err(Error::ResourceLimit("max_pdf_stream_bytes"));
            }
    ...
    fn decompress_predictor(mut data: Vec<u8>, params: Option<&Dictionary>) -> Result<Vec<u8>> {
        ...
        if let Some(params) = params {
            ...
            if (10..=15).contains(&predictor) {
                let pixels_per_row = usize::try_from(max(
                    1,
                    params.get(b"Columns").and_then(Object::as_i64).unwrap_or(1),
                ))?;
                let colors = usize::try_from(max(
                    1,
                ...
                let bytes_per_pixel = colors
                    ...
                    .ok_or(Error::ResourceLimit("max_pdf_stream_bytes"))?;
                let bytes_per_row = bytes_per_pixel
                    .checked_mul(pixels_per_row)
                    .ok_or(Error::ResourceLimit("max_pdf_stream_bytes"))?;
                if bytes_per_row > MAX_DECOMPRESSED_STREAM_BYTES {
                    return Err(Error::ResourceLimit("max_pdf_stream_bytes"));
                }
...
"
    ...
    #[test]
    fn compressed_stream_bomb_stops_at_mobile_output_limit() {
        use flate2::Compression;
        use flate2::write::ZlibEncoder;
        use std::io::Write;
        ...
        let mut encoder = ZlibEncoder::new(Vec::new(), Compression::best());
        encoder
            .write_all(&vec![b'x'; MAX_DECOMPRESSED_STREAM_BYTES + 1])
            .unwrap();
        let compressed = encoder.finish().unwrap();
        assert!(compressed.len() < MAX_DECOMPRESSED_STREAM_BYTES / 100);
        let mut dict = crate::Dictionary::new();
        dict.set("Filter", "FlateDecode");
        let stream = Stream::new(dict, compressed);
        assert!(matches!(
            stream.decompressed_content(),
            Err(Error::ResourceLimit("max_pdf_stream_bytes"))
        ));
    }

    #[test]
    fn predictor_dimensions_cannot_allocate_past_stream_limit() {
        let mut params = crate::Dictionary::new();
        params.set("Predictor", 12);
        params.set("Columns", i64::MAX);
        let error = Stream::decompress_predictor(vec![0], Some(&params)).unwrap_err();
        assert!(matches!(error, Error::ResourceLimit("max_pdf_stream_bytes")));
    }
}
