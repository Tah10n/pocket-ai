...
/// Return whether `bytes` are an OLE compound container carrying the
/// `EncryptionInfo`/`EncryptedPackage` streams used for password-protected
/// OOXML. This bounded probe lets a native frontend preserve the stable
/// encrypted-document error before the inner DOCX/XLSX/PPTX format is
/// available for normal content detection.
pub fn is_encrypted_ooxml_container(bytes: &[u8]) -> bool {
    matches!(
        package::archive::probe_ole(bytes),
        Some(ConvertError::Encrypted)
    )
}
