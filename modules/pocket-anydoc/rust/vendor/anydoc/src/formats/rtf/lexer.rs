...
pub fn destination_groups<'a>(
    bytes: &'a [u8],
    name: &str,
) -> Result<Vec<&'a [u8]>, crate::error::ConvertError> {
    let mut out = Vec::new();
...
mod tests {
    ...
    fn destination_extraction() {
        ...
        let lists = destination_groups(src, "listtable").unwrap();
        assert_eq!(lists.len(), 1);
        assert!(lists[0].starts_with(br"{\list"));
        let fonts = destination_groups(src, "fonttbl").unwrap();
