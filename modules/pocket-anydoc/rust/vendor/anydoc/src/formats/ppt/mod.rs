...
pub fn parse(bytes: &[u8]) -> Result<Document, ConvertError> {
    crate::runtime::charge(1 + bytes.len().div_ceil(4096))?;
...
fn collect_pictures<R: std::io::Read + std::io::Seek>(
    ...
    while let Some((ver_inst, rec_type, body)) = officeart::record_at(&pictures, pos) {
        crate::runtime::charge(1 + body.len().div_ceil(4096))?;
        pos += 8 + body.len();
        index += 1;
        if index > 100_000 {
            break;
...
impl Extractor {
    ...
    fn end_segment(&mut self, id: Option<u32>) -> Result<(), ConvertError> {
        self.flush_shape()?;
    ...
    fn into_blocks(mut self) -> Result<Vec<Block>, ConvertError> {
        ...
        for (sid, blocks) in slides {
            crate::runtime::charge(1)?;
            let mut slide_blocks = blocks;
            for (i, (nid, nblocks)) in notes.iter_mut().enumerate() {
                if !used[i] && sid.is_some() && *nid == sid {
                    used[i] = true;
                    slide_blocks.push(Block::BlockQuote(std::mem::take(nblocks)));
                }
            }
    ...
    fn charge_record(&mut self, body_len: usize) -> Result<(), ConvertError> {
        crate::runtime::charge(1 + body_len.div_ceil(4096))?;
        self.records += 1;
    ...
    fn atom(&mut self, rec_type: u16, body: &[u8]) -> Result<(), ConvertError> {
        match rec_type {
            ...
            0x0FA0 => {
                ...
                for (index, decoded) in char::decode_utf16(units).enumerate() {
                    ...
                    text.push(decoded.unwrap_or('\u{fffd}'));
                }
                self.push_text(text);
            }
    ...
    fn flush_shape(&mut self) -> Result<(), ConvertError> {
        ...
        for (char_index, c) in shape.text.chars().enumerate() {
            if char_index % 256 == 0 {
                crate::runtime::charge(1)?;
            }
