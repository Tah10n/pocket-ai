...
use crate::{
    mobile_sparse_range_limit, Cell, Data, HeaderRow, Metadata, Range, Reader, ReaderRef, Sheet,
    SheetType, SheetVisible, MOBILE_CELL_LIMIT_MESSAGE, MOBILE_MAX_WORKSHEET_CELLS,
};
...
impl std::fmt::Display for XlsbError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            XlsbError::CellLimitExceeded { requested, max } => write!(
                f,
                ...
                0x0269 => {
                    ...
                    for _ in 0..len {
                        ...
                        if fmt_code >= 164 {
                            ...
                            self.formats.push(builtin_format_by_code(fmt_code));
                        }
                    }
    ...
    fn worksheet_formula(&mut self, name: &str) -> Result<Range<String>, XlsbError> {
        ...
        let mut cells = Vec::with_capacity(
            cells_reader
                .dimensions()
                .len()
                .min(MOBILE_MAX_WORKSHEET_CELLS as u64) as _,
        );
...
fn ensure_mobile_vector_len(len: usize) -> Result<(), XlsbError> {
    if len > MOBILE_MAX_WORKSHEET_CELLS {
        Err(XlsbError::CellLimitExceeded {
            requested: len,
...
fn ensure_mobile_sparse_range<T: crate::CellType>(cells: &[Cell<T>]) -> Result<(), XlsbError> {
    mobile_sparse_range_limit(cells).map_err(|requested| XlsbError::CellLimitExceeded {
        requested,
        max: MOBILE_MAX_WORKSHEET_CELLS,
    })
}
