...
fn scan_codepage(bytes: &[u8]) -> Result<&'static encoding_rs::Encoding, ConvertError> {
    ...
    while let Some(token) = lexer.next_token() {
        ...
        {
            return Ok(codepage_encoding(cp.max(0) as u32));
        }
    }
...
impl PictState {
    ...
    fn payload(self) -> Result<Vec<u8>, ConvertError> {
        if let Some(binary) = self.binary {
            crate::runtime::charge(1 + binary.len().div_ceil(4096))?;
            return Ok(binary);
        }
        let mut out = Vec::with_capacity(self.hex.len() / 2);
        let mut high: Option<u8> = None;
        for (index, &b) in self.hex.iter().enumerate() {
            if index % 512 == 0 {
                crate::runtime::charge(1)?;
            }
    ...
    fn run(&mut self) -> Result<(), ConvertError> {
        ...
        while let Some(token) = self.lexer.next_token() {
            ...
            match token {
                ...
                Token::Bin(payload) => {
                    crate::runtime::charge(1 + payload.len().div_ceil(4096))?;
                    if self.state.capture == Capture::Pict
