...
use crate::{
    mobile_merged_range_limit, mobile_sparse_range_limit, Cell, CellErrorType, Data, Dimensions,
    HeaderRow, Metadata, Range, Reader, Sheet, SheetType, SheetVisible, MOBILE_CELL_LIMIT_MESSAGE,
    MOBILE_MAX_MERGED_REGIONS, MOBILE_MAX_WORKSHEET_CELLS,
};
...
pub enum XlsError {
    ...
    /// Workbook exceeds Pocket AI's bounded mobile worksheet profile.
    CellLimitExceeded {
        /// Sparse cells or dense cells requested.
        requested: usize,
...
impl std::fmt::Display for XlsError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            XlsError::CellLimitExceeded { requested, max } => write!(
                f,
                "{MOBILE_CELL_LIMIT_MESSAGE} ({requested} cells requested, max {max})"
            ),
        }
    }
}
            ...
            for record in records {
                ...
                match r.typ {
                    ...
                    0x0200 => {
                        ...
                        cells.reserve(rows.saturating_mul(cols).min(MOBILE_MAX_WORKSHEET_CELLS));
                    }
                ...
                ensure_mobile_vector_len(cells.len())?;
                ensure_mobile_vector_len(formulas.len())?;
            }
            ensure_mobile_sparse_range(&cells)?;
            ensure_mobile_sparse_range(&formulas)?;
...
fn parse_merge_cells(r: &[u8], merge_cells: &mut Vec<Dimensions>) -> Result<(), XlsError> {
    ...
    let expected = 2usize.saturating_add(count.saturating_mul(8));
    if r.len() < expected {
        return Err(XlsError::Len {
            typ: "merged cells",
            expected,
    ...
    if merge_cells.len().saturating_add(count) > MOBILE_MAX_MERGED_REGIONS {
        return Err(XlsError::CellLimitExceeded {
            requested: merge_cells.len().saturating_add(count),
            max: MOBILE_MAX_MERGED_REGIONS,
        });
    }
    ...
    for i in 0..count {
        ...
        if let Err(requested) = mobile_merged_range_limit(&dimensions) {
            return Err(XlsError::CellLimitExceeded {
                requested,
                max: MOBILE_MAX_WORKSHEET_CELLS,
            });
        }
...
fn ensure_mobile_vector_len(len: usize) -> Result<(), XlsError> {
    if len > MOBILE_MAX_WORKSHEET_CELLS {
        Err(XlsError::CellLimitExceeded {
            requested: len,
            max: MOBILE_MAX_WORKSHEET_CELLS,
        })
    } else {
...
fn ensure_mobile_sparse_range<T: crate::CellType>(cells: &[Cell<T>]) -> Result<(), XlsError> {
    mobile_sparse_range_limit(cells).map_err(|requested| XlsError::CellLimitExceeded {
        requested,
        max: MOBILE_MAX_WORKSHEET_CELLS,
    })
}
