...
pub fn parse(bytes: &[u8]) -> Result<Document, ConvertError> {
    ...
    let spine_hrefs: Vec<&str> = opf
        ...
        .collect();
    if spine_hrefs.len() > crate::package::limits::MAX_EPUB_SPINE_ITEMS {
        return Err(ConvertError::ResourceLimit {
            limit: "max_epub_spine_items",
            detail: "EPUB spine exceeds the mobile work budget".into(),
        });
    }
    let mut repeated_parts: HashMap<&str, usize> = HashMap::new();
    for href in &spine_hrefs {
        crate::runtime::charge(1)?;
        let seen = repeated_parts.entry(href).or_default();
        *seen += 1;
        if *seen > crate::package::limits::MAX_EPUB_REPEATED_ITEMREFS {
            return Err(ConvertError::ResourceLimit {
                limit: "max_epub_repeated_itemrefs",
                detail: "EPUB repeatedly references one spine part".into(),
            });
        }
    }
