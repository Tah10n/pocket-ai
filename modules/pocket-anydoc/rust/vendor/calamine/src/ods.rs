...
use crate::{
    Data, DataType, HeaderRow, Metadata, Range, Reader, Sheet, SheetType, SheetVisible,
    MOBILE_CELL_LIMIT_MESSAGE, MOBILE_MAX_WORKSHEET_CELLS,
};
...
const MAX_CELLS: usize = MOBILE_MAX_WORKSHEET_CELLS;
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(e)) if e.name() == QName(b"table:table-row") => {
                ...
                // Even empty repeated rows consume parser metadata and can
                // turn a later cell into an enormous dense range. Reject the
                // requested logical span instead of silently truncating it.
                let requested_rows = total_rows.saturating_add(row_repeats);
                let max_logical_rows = (MAX_ROWS as usize).min(MAX_CELLS);
                if requested_rows > max_logical_rows {
                    return Err(OdsError::CellLimitExceeded {
                        requested: requested_rows,
                        max: max_logical_rows,
                    });
                }
                ...
                if cells.len() > MAX_CELLS || formulas.len() > MAX_CELLS {
                    return Err(OdsError::CellLimitExceeded {
                        requested: cells.len().max(formulas.len()),
                        max: MAX_CELLS,
                    });
                }
                cols.push(cells.len());
    ...
    loop {
        ...
        match reader.read_event_into(row_buf) {
                    ...
                    || e.name() == QName(b"table:covered-table-cell") =>
            {
                ...
                let row_span: usize = match e.raw_attr(b"table:number-rows-spanned")? {
                    Some(v) => reader.decoder().decode(v)?.parse()?,
                    None => 1,
                };
                let column_span: usize = match e.raw_attr(b"table:number-columns-spanned")? {
                    Some(v) => reader.decoder().decode(v)?.parse()?,
                    None => 1,
                };
                let span_cells = row_span.saturating_mul(column_span);
                if span_cells > MAX_CELLS {
                    return Err(OdsError::CellLimitExceeded {
                        requested: span_cells,
                        max: MAX_CELLS,
                    });
                }
                ...
                // A provider-controlled repeat must not be silently clipped:
                // the caller needs a stable resource-limit error.
                let current_cols = cells.len() - row_start;
                let remaining = (MAX_COLUMNS as usize).saturating_sub(current_cols);
                if empty_col_repeats > remaining {
                    return Err(OdsError::CellLimitExceeded {
                        requested: current_cols.saturating_add(empty_col_repeats),
                ...
                // Cap the physical row before cloning cell/formula values.
                let current_cols = cells.len() - row_start;
                let remaining = (MAX_COLUMNS as usize).saturating_sub(current_cols);
                if repeats > remaining {
                    return Err(OdsError::CellLimitExceeded {
                        requested: current_cols.saturating_add(repeats),
                        max: MAX_COLUMNS as usize,
                    });
                }
            ...
            Ok(Event::Text(e)) if e.iter().all(|byte| byte.is_ascii_whitespace()) => (),
            Ok(Event::Comment(_)) => (),
            Err(e) => return Err(OdsError::Xml(e)),
            Ok(e) => {
