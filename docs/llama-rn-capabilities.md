...
permission snapshots, bounds and recovery behavior. `LocalToolRun` extends the existing
chat path, with one engine owner across native completions and document work.
`LocalToolExecutor` validates complete final arguments against fixed local schemas;
`DocumentToolSearch` checks current-chat ownership before and after reads. Persistent
protocol groups restore unfinished runs as interrupted and never replay actions.

`PreparedLlamaRequest` and exact counts include definitions, choice, phase, protocol
messages and results. Selection keeps tool grammar/parser metadata; final JSON/GBNF
restores the existing independent constraint and validator, with `tool_choice: none`.
Definitions remain in final formatting to select the same tool-use template.

The installed `cpp/common/chat.cpp` choice parser accepts `auto`, `none`, `required`
only. `required` is first-step-only at the app layer. The JS wrapper stringifies
`parallel_tool_calls`, but `JSIParams.cpp` reads a strict boolean, so omission retains
false. The source patch does not change this behavior. `context.parallel` remains off.
The README's older `GENERIC`/`{"response":"..."}` fallback is absent from the actual
rc.3 PEG implementation. Compatible differential autoparsers are supported; arbitrary
JSON extraction and silent protocol-to-user/system fallback are not.

Template capability and successful formatting are distinct from model suitability
and real native execution. Stage 4 native acceptance is **not_run pending acceptance**:
model-generated calls, real execution/result continuation, document search, final
JSON Schema, Stop/retry and cold history reopen must be recorded separately. Earlier
Stage 1-3 receipts below remain historical evidence for their exact sources/APKs.
...
| [Tools](https://github.com/mybigday/llama.rn/blob/v0.13.0-rc.3/src/index.ts#L275): `tools`, `tool_choice`, `parallel_tool_calls`, streamed/final `tool_calls` | Stage 4 adds typed persisted call/result groups, the static local allowlist, sequential execution and bounded result-to-model continuation. See [Local tools](local-tools.md). | Compatible Jinja tool capabilities and native parser metadata are required; differential autoparsers are eligible. Only `auto`, `none`, `required` choices exist. The rc.3 wrapper makes `parallel_tool_calls` effectively false. Partial arguments remain non-executable strings. | **1** preserves the contract; **4** implements the loop. Application tests cover executor, dispatch, cancellation and formatting. Real model-generated native calls and continuation remain pending acceptance. |
...
| **4 — Tools** | Implemented bounded local allowlist, validated final calls, sequential execution, canonical history and result-to-model loop. Application tests pass; native acceptance pending. Partial streams remain non-executable. |
