...
| [Tools](https://github.com/mybigday/llama.rn/blob/v0.13.0-rc.3/src/index.ts#L275): `tools`, `tool_choice`, `parallel_tool_calls`, streamed/final `tool_calls` | Stage 4 adds typed persisted call/result groups, the static local allowlist, sequential execution and bounded result-to-model continuation. See [Local tools](local-tools.md). | Compatible Jinja tool capabilities and native parser metadata are required; differential autoparsers are eligible. Only `auto`, `none`, `required` choices exist. The rc.3 wrapper makes `parallel_tool_calls` effectively false. Partial arguments remain non-executable strings. | **1** preserves the contract; **4** implements the loop. Application tests cover executor, dispatch, cancellation and formatting. Required native calls and continuation passed the documented Android CPU fixture; automatic calculator non-selection is reported separately. |
