...
In GitHub PRs, `Run Android checks` runs the fail-closed `runtime` pack by default. `Run Android scenarios` keeps the legacy extended pack. `Run Android document pack` or the `android-pack-documents` label runs the release document pack with synthetic fixtures and a commit-pinned public QA model; the shipping-build guard rejects that QA bootstrap from distributable builds. Maintainers can also apply `android-pack-all`, `android-pack-native`, `android-pack-runtime`, `android-pack-dependency-ui`, `android-pack-catalog`, or `android-pack-extended`. The `document-benchmark` pack remains an explicit maintainer/device command, and `documents` remains excluded from `android-pack-all`. If multiple pack labels are present, CI uses the first match in this priority order: `android-pack-all`, `android-pack-documents`, `android-pack-native`, `android-pack-runtime`, `android-pack-dependency-ui`, `android-pack-catalog`, then `android-pack-extended`.
