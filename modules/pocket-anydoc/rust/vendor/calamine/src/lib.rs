...
mod datatype;
mod formats;
pub use formats::UNSUPPORTED_FORMAT_SENTINEL;
mod ods;
mod xls;
mod xlsb;
mod xlsx;
...
/// Pocket AI's mobile ceiling for both retained sparse cells and the dense
/// rectangle materialized by `Range::from_sparse`.  Keeping this limit in the
/// parser (rather than only in its caller) prevents a malicious workbook from
/// allocating the range before the host can account for it.
pub(crate) const MOBILE_MAX_WORKSHEET_CELLS: usize = 250_000;

/// A workbook can describe many tiny or overlapping merged regions without
/// adding normal cells. Bound their metadata independently.
pub(crate) const MOBILE_MAX_MERGED_REGIONS: usize = 4_096;

/// Stable marker consumed by the bounded anydoc adapter. The remainder of a
/// parser error can change without changing the public resource-limit class.
pub const MOBILE_CELL_LIMIT_MESSAGE: &str = "Pocket mobile cell limit exceeded";

/// Return the number of cells that would be allocated for this sparse range
/// when it exceeds the mobile ceiling. This must be called before
/// `Range::from_sparse` by every workbook parser.
pub(crate) fn mobile_sparse_range_limit<T: CellType>(cells: &[Cell<T>]) -> Result<(), usize> {
    if cells.len() > MOBILE_MAX_WORKSHEET_CELLS {
        return Err(cells.len());
    }
    let Some(first) = cells.first() else {
        return Ok(());
    };
    let (mut row_min, mut col_min) = first.pos;
    let (mut row_max, mut col_max) = first.pos;
    for cell in &cells[1..] {
        row_min = row_min.min(cell.pos.0);
        row_max = row_max.max(cell.pos.0);
        col_min = col_min.min(cell.pos.1);
        col_max = col_max.max(cell.pos.1);
    }
    let rows = (row_max as usize)
        .saturating_sub(row_min as usize)
        .saturating_add(1);
    let cols = (col_max as usize)
        .saturating_sub(col_min as usize)
        .saturating_add(1);
    let requested = rows.saturating_mul(cols);
    if requested > MOBILE_MAX_WORKSHEET_CELLS {
        Err(requested)
    } else {
        Ok(())
    }
...
pub(crate) fn mobile_merged_range_limit(range: &Dimensions) -> Result<(), usize> {
    let rows = (range.end.0 as usize)
        .saturating_sub(range.start.0 as usize)
        .saturating_add(1);
    let cols = (range.end.1 as usize)
        .saturating_sub(range.start.1 as usize)
        .saturating_add(1);
    let requested = rows.saturating_mul(cols);
    if requested > MOBILE_MAX_WORKSHEET_CELLS {
        Err(requested)
...
#[cfg(test)]
mod mobile_limit_tests {
    use super::*;

    #[test]
    fn sparse_extent_is_checked_before_dense_allocation() {
        let cells = vec![
            Cell::new((0, 0), Data::Int(1)),
            Cell::new((MOBILE_MAX_WORKSHEET_CELLS as u32, 1), Data::Int(2)),
        ];
        assert!(matches!(
            mobile_sparse_range_limit(&cells),
            Err(requested) if requested > MOBILE_MAX_WORKSHEET_CELLS
        ));
    }

    #[test]
    fn sparse_extent_at_mobile_ceiling_is_allowed() {
        let cells = vec![
            Cell::new((0, 0), Data::Int(1)),
            Cell::new((499, 499), Data::Int(2)),
        ];
        assert_eq!(mobile_sparse_range_limit(&cells), Ok(()));
    }

    #[test]
    fn merged_area_is_checked_without_expansion() {
        let range = Dimensions {
            start: (0, 0),
            end: (500, 500),
        };
        assert!(matches!(
            mobile_merged_range_limit(&range),
            Err(requested) if requested > MOBILE_MAX_WORKSHEET_CELLS
        ));
    }
}
